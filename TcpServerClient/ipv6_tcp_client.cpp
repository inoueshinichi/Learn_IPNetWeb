...
struct sockaddr_in6 *p_server; // ネットワーク用インターフェース /* IPv6 */
// struct hostent *host; // IPv4 ホスト
struct struct addrinfo *host; // IPv6 ホスト
socklen_t socket_length;
unsigned short port_of_server = 12345;
...
int main(int argc, char **argv)
{
    try
    {
        ...
        /* 2.接続先指定用構造体の準備 */
        p_server = (struct sockaddr_in6 *)&server_info; // /* IPv6 */ `sockeaddr`構造体を`sockaddr_in*`構造体ポインタを通して使用する.
        p_server->sin6_family = AF_INET6; /* IPv6 */
        p_server->sin6_port = htons(port_of_server); /* IPv6 */

        /* 3.宛先(server)の確定 */
        // char *server_name = "127.0.0.1"; /* IPv4 */
        char *server_name = "::1"; // loopback /* IPv6 */

        // p_server->sin6_addr = in6addr_loopback;
        // if (p_server->sin6_addr == INADDR_NONE)
        if (inet_pton(AF_INET6, server_name, &(p_server->sin6_addr)) == INADDR_NONE) /* IPv6 */
        //if ((p_server->sin6_addr.s6_addr = inet_addr(server_name)) == INADDR_NONE) // IPv6では, `inet_addr`は使えない.
        {
            // IPアドレスではない
